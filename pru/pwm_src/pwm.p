...
INNER_LOOP:
        QBNE CHECK1, r0, channels.ch1*US_TO_CYCLES //check if since start pulsewidth
                                      //equal to CH1 has been sent
        CLR r30.t8                    //if it has, set CH1 low

CHECK1:
        QBNE CHECK2, r0, channels.ch2*US_TO_CYCLES //check if since start pulsewidth
                                      //equal to CH2 has been sent
        CLR r30.t10         //if it has, set CH2 low

CHECK2:
        QBNE CHECK3, r0, channels.ch3*US_TO_CYCLES //check if since start pulsewidth
                                      //equal to CH3 has been sent
        CLR r30.t9          //if it has, set it low

CHECK3:
        QBNE FINALCHECK, r0, channels.ch4*US_TO_CYCLES //check if since start pulsewidth
                                      //equal to CH4 has been sent
        CLR r30.t11          //if it has, set it low

FINALCHECK:
        QBEQ READ_MEM, r0, channels.max_pulse*US_TO_CYCLES //if total period is over, read new values in
